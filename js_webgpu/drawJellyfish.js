...
function drawJellyfish(renderPassDescriptor) {
  interpolateTargets();

  var commandEncoder = device.createCommandEncoder({});
  const passEncoder = commandEncoder.beginRenderPass(renderPassDescriptor);

  setMatrixUniforms(passEncoder);

  //bindTexture('caustics'+localParam.cycle32, 1);
  jellyfish.order.sort(sort3D);

  var curJellyFishCount = jellyfish.count;
  allocateJellyFishUniformBuffer(curJellyFishCount);

  bindTextureBindGroup(passEncoder, 'jellyfish', 2);

  passEncoder.setPipeline(renderPipeline);

  for (var i=0; i < curJellyFishCount; i++) {
    var k = jellyfish.order[i][0];
    if (jellyfish[k]){
      jellyfish[k].simulate();
      jellyfish[k].setLOD();
      jellyfish[k].draw(i, passEncoder);
    }
  }
...
function sort3D(a,b){
  ...
  function JellyfishInstance(pos,scl,time){
    ...
    this.draw = function(i, passEncoder){
        this.propel = (Math.sin(this.time+Math.PI)+0.6)*0.2;
        setPerFishUniforms(passEncoder, i, this.time);
        bindTextureBindGroup(passEncoder, 'caustics'+localParam.cycle32, 3);

        drawBuffer(passEncoder, 'jellyfish'+this.lod);
    };
